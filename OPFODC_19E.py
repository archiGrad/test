...
DEFAULTS = {
    ...
    'PIPELINE_ORDER': ['sharpen', 'blur', 'contrast', 'exposure', 'gamma', 'color_to_transparent', 'alpha_outline', 'dither'],
...
function createSpritesheetBg(container, images, SS, scene, renderer, opacity, cropConfig) {{
    ...
    const slotId = (s) => s.key + ':' + s.px + ',' + s.py;

    const showPreview = (slot) => {{
        if (!preview) return;
        const imgEl = wrap.querySelector('img[data-ss-key="' + slot.key + '"]');
        if (!imgEl) return;
        const ir = imgEl.getBoundingClientRect();
        const er = el.getBoundingClientRect();
        const narrow = ir.width < 80 || ir.height < 80;
        if (narrow) {{
        ...
        const pct = parseFloat(getComputedStyle(preview).getPropertyValue('--ss-crop-size') || '50') / 100;
        const displaySz = ir.width * pct;
    ...
    const onClick = (e) => {{
        ...
        const id = slotId(currentSlot);
        if (pinnedId === id) {{ hidePreview(); return; }}
        showPreview(currentSlot);
    }};
