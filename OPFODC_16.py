...
# ==========================================
TEXT_IMAGE_RESOLUTION = 100
KTX2_ENCODE = 'uastc'   # 'uastc' (high quality, larger files) or 'etc1s' (smaller files, lower quality). Default: 'uastc'
KTX2_QUALITY = 2        # uastc: 0-4 (0=fastest, 4=best). etc1s: 1-255 (higher=better). Default: 2
KTX2_ZCMP = 5           # Zstandard supercompression level 1-20 (higher=smaller but slower). Ignored for etc1s. Default: 5
...
function createSpritesheetBg(container, images, SS, scene, renderer, opacity) {{
    ...
    images.forEach(img => {{
        ...
        const idx = img.anim && img.fm ? img.fm[0] : img.idx;
    ...
    function update(camera) {{
        ...
        if (!meshCache) {{
            meshCache = [];
            scene.traverse(obj => {{ if (obj.isMesh) meshCache.push(obj); }});
        }}
        ...
        if (hits.length > 0 && hits[0].uv) {{
            ...
            if (!wireframe) {{
                ...
                hit.object.parent.add(wireframe);
            }} else {{
