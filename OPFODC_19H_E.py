...
def manage_folder_labels(node, parent_config):
    ...
    if should_generate and has_real_images:
        ...
        show_cp = current_config.get('IMAGE_SETTINGS_CP', False)
        ...
        # -------------------------------------------------
        # PRIORITY LOGIC
        # 1) IMAGE_SETTINGS_CP=True -> show .custom_processing content
        # 2) IMAGE_SETTINGS_CUSTOMTEXT non-empty -> use that
        # 3) dotfile -> use dotfile
        # 4) fallback -> folder name
        # -------------------------------------------------
        if show_cp:
            cp_file = node_path / '.custom_processing'
            if cp_file.exists():
